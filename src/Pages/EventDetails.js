...
import { Container, Typography, TextField, Box, FormControl, InputLabel, Select, MenuItem, FormHelperText, Button, Card, CardContent, Grid, CircularProgress, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Snackbar, Alert, IconButton } from '@mui/material';
...
const EventDetails = () => {
  ...
  const validateForm = () => {
    ...
    if (!eventData.eventDescription) newErrors.eventDescription = 'Event description is required.';
    if (!eventData.location) newErrors.location = 'Location is required.';
    if (eventData.requiredSkills.length === 0) newErrors.requiredSkills = 'At least one skill is required.';
  ...
  return (
    <Container sx={{ mt: 5, mb: 5 }}>
      <StyledCard>
        <CardContent>
        <BackButton onClick={() => navigate('/event-management')}>
          <ArrowBackIcon />
        </BackButton>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} textAlign="center">
                <StyledButton type="submit" disabled={isSubmitting}>
                  {isSubmitting ? <CircularProgress size={24} /> : 'Update Event'}
                </StyledButton>
              </Grid>
            </Grid>
          </Box>
